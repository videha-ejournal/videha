...
function ensureMeta(name,content){if(!document.head)return;let m=document.head.querySelector('meta[name="'+name+'"]');if(!m){m=document.createElement("meta");m.setAttribute("name",name);document.head.appendChild(m);}m.setAttribute("content",content);}
function addText(parent,text){const span=document.createElement("span");span.textContent=text;parent.appendChild(span);}
function addLink(parent,label,url){const span=document.createElement("span");if(label)span.appendChild(document.createTextNode(label+" "));const a=document.createElement("a");a.href=url;a.textContent=url;span.appendChild(a);parent.appendChild(span);}
function initScholarlyIdentity(){
  ...
  addText(aside,"Videha — First Maithili Fortnightly eJournal");
  addText(aside,"ISSN "+ISSN);
  addLink(aside,"Primary:",PRIMARY);
  addLink(aside,"GitHub mirror:",GITHUB);
  addLink(aside,"Digital Research Archive:",RESEARCH);
  addLink(aside,"GitHub research network:",GH_ORG);
