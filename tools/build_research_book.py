...
def _author_display(name):
    ...
    return re.sub(r'^(डॉ०|डॉ\.?|डाॅ\.?|डा\.?|प्रो\.?|आचार्य|श्री|श्रीमती|पं\.?|पण्डित)(?=\S)', r'\1 ', str(name or '').strip(), flags=re.I)
...
def sortkey(a):
    ...
    return re.sub(r'^(डॉ०|डॉ\.?|डाॅ\.?|डा\.?|प्रो\.?|आचार्य|श्री|श्रीमती|पं\.?|पण्डित)\s*','',auth(a),flags=re.I).casefold()
...
def long_narrative(data):
    out = list(NARRATIVE)
    by_issue = defaultdict(list)
    for a in data:
        try: by_issue[int(a.get('issue'))].append(a)
        except (TypeError, ValueError): pass
    for lo in range(1, 448, 10):
        hi = min(lo + 9, 447); subset = [a for i in range(lo, hi + 1) for a in by_issue.get(i, [])]
        if not subset: continue
        langs = defaultdict(int); gs = defaultdict(int); authors = defaultdict(int)
        for a in subset:
            langs[a.get('language') or 'mai'] += 1; gs[genre(a)] += 1
            for au in a.get('authors') or ['अज्ञात लेखक']: authors[au] += 1
        langtxt = ' · '.join(f'{k} {v}' for k,v in sorted(langs.items()))
        gentxt = ', '.join(f'{k} ({v})' for k,v in sorted(gs.items(), key=lambda x:(-x[1],x[0].casefold()))[:8])
        autxt = ', '.join(a for a,_ in sorted(authors.items(), key=lambda x:(-x[1],x[0]))[:8])
        out.append((f'अंक {lo}–{hi} : दशक-दशकक पाठ', f'अंक {lo} सँ {hi} धरि {len(subset)} सत्यापित शोध-लेख भेटैत अछि। एहि खण्डमे भाषिक वितरण {langtxt} अछि; प्रमुख विधा {gentxt} छथि। बारम्बार उपस्थित लेखक {autxt} छथि। एहि दशक-दशकक पाठसँ बुझाइत अछि जे विदेहक सम्पादकीय रुचि एकरेखीय नहि रहल—कखन भाषा-आन्दोलन आ साहित्यिक आलोचना केन्द्रमे रहल, कखन लोक-अध्ययन, इतिहास, कानून, नारी-विमर्श अथवा विज्ञान। प्रत्येक लेख अपन अंकक सामाजिक समय, प्रकाशनक उद्देश्य आ लेखकक शोध-दृष्टिक साक्षी अछि।'))
        titles = ' · '.join((a.get('title') or '')[:110] for a in subset[:12])
        out.append((f'अंक {lo}–{hi} : पाठ-समूह आ स्रोत', f'एहि खण्डक प्रत्यक्ष पाठ सभमे {titles} सन शीर्षक भेटैत अछि। शीर्षकक संग लेखक-नाम, तिथि, अंक-संख्या, पृष्ठ-संकेत आ वर्गीकरण जोड़ि पढ़ला पर शोध-परम्पराक क्रमिक रूप स्पष्ट होइत अछि। आरम्भिक लेख सभ बहुधा परिचयात्मक वा आन्दोलन-केन्द्रित छथि, पश्चातक लेख स्रोत, प्रतिपक्ष, क्षेत्र-अनुभव आ तुलनात्मक विवेचन बढ़बैत छथि। ई पाठ-समूह भविष्यक शोधकर्ताकेँ उद्धरणक प्राथमिक स्थान, पुनर्पाठक अवसर आ अनुपस्थित प्रश्न—तीनू देखबैत अछि।'))
    for g in sorted({genre(a) for a in data}, key=str.casefold):
        items = [a for a in data if genre(a)==g]; sample = ' · '.join((a.get('title') or '')[:90] for a in items[:10])
        out.append((f'विधा-विशेष अध्ययन : {g}', f'{g} वर्गमे {len(items)} लेख अछि। एहि वर्गक लेख सभक पाठमे विषय-चयन, तर्कक ढाँचा, स्रोतक उपयोग आ मैथिलीमे ज्ञान-निर्माणक भिन्न-भिन्न रूप देखाइत अछि। प्रतिनिधि शीर्षक सभ—{sample}—सँ स्पष्ट अछि जे विदेह केवल साहित्यिक समीक्षा नहि, बल्कि इतिहास, समाज, भाषा, कला, कानून, पर्यावरण आ अन्तरविषयी अध्ययनक साझा मंच अछि। विधा-वार पाठ शोधकर्ताकेँ तुलनात्मक प्रश्न बनबैत, लेख सभक पारस्परिक सम्बन्ध बुझबैत आ नव शोधक सम्भावित दिशा देखबैत अछि।'))
        langs = defaultdict(int); issues = defaultdict(int)
        for a in items: langs[a.get('language') or 'mai'] += 1; issues[str(a.get('issue') or '—')] += 1
        out.append((f'{g} : भाषा आ अंक-वितरण', f'{g}क लेख सभमे भाषा-वितरण {", ".join(f"{k} {v}" for k,v in sorted(langs.items()))} अछि आ ई {len(issues)} अलग अंकमे पसारल अछि। एहि फैलावसँ विधाक निरन्तरता आ परिवर्तन दुनू देखाइत अछि। एकर पाठक तुलना करैत काल शीर्षक मात्र नहि, लेखकक पद्धति, उद्धृत परम्परा, स्थानीय उदाहरण, शब्द-चयन आ निष्कर्षक सामाजिक अर्थ सेहो देखबाक चाही।'))
    return out
...
# The print-ready DOCX retains the full scholarly narrative.  The HTML landing
# page is intentionally compact for mobile: its editorial note is followed
# directly by the two complete annexure tables.
NARRATIVE_FULL = long_narrative(DATA)
HTML_NARRATIVE_FULL = []
LANG_LABELS={'mai':'मैथिली','en':'अंग्रेजी','sa':'संस्कृत','thethi-angika':'ठेठी-अंगिका','bajjika':'बज्जिका'}
LANG_COUNTS=defaultdict(int)
for _a in DATA: LANG_COUNTS[_a.get('language') or 'mai'] += 1
parts=['<!doctype html><html lang="mai"><head><meta charset="utf-8"><title>विदेह शोध-लेख : अंक १ सँ ४४७ धरि</title><style>@page{size:letter;margin:18mm}body{font-family:"Noto Serif Devanagari","Nirmala UI",serif;color:#18212b;line-height:1.5}h1{color:#163a5f}h2{color:#1f4d78;border-bottom:1px solid #b9c8d8}p{font-size:12px;text-align:justify}.home-strip{display:flex;flex-wrap:wrap;gap:.8rem;padding:9px 12px;margin:10px 0;background:#faf6ee;border:1px solid #d8cfc0;font-weight:700}.home-strip a{color:#8b1a1a}.book-tools{display:flex;flex-wrap:wrap;gap:.6rem;align-items:center;background:#faf6ee;border:1px solid #d8cfc0;padding:10px;margin:14px 0}.book-tools input{flex:1 1 18rem;padding:8px;font:inherit}.book-tools button{padding:7px 10px;font:inherit;cursor:pointer}.book-status{font-size:.9rem;color:#555}.videha-sr-only{position:absolute;width:1px;height:1px;margin:-1px;overflow:hidden;clip:rect(0,0,0,0)}.back-top{position:fixed;right:20px;bottom:20px;width:46px;height:46px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:linear-gradient(145deg,#e53935,#8b0000);box-shadow:inset 2px 2px 5px #ff8a80, inset -3px -3px 6px #4a0000, 0 3px 8px #777;color:#fff;font-size:28px;font-weight:900;z-index:10}.back-top a{color:#fff!important;text-decoration:none}table{border-collapse:collapse;width:100%;font-size:8.5px;page-break-inside:auto}thead{display:table-header-group}tr{page-break-inside:avoid}th,td{border:1px solid #b7c4d1;padding:4px;vertical-align:top}th{background:#e8eef5;color:#173b5f}a{color:#155a8a;text-decoration:none}</style></head><body><div class="home-strip">⭐ <a href="https://www.videha.co.in/">VIDEHA Home · www.videha.co.in</a> · ⭐ <a href="https://videha-ejournal.github.io/videha/">GitHub Home · videha-ejournal.github.io/videha</a></div>']
parts[0] = parts[0].replace('</style>', '@media(max-width:600px){body{padding:12px;font-size:16px}p{font-size:1rem;text-align:left}.home-strip{gap:.45rem;padding:8px;font-size:.9rem}.book-tools{margin:10px 0;padding:8px}.book-tools input{flex-basis:100%;width:100%}table{display:block;overflow-x:auto;max-width:100%;font-size:12px;-webkit-overflow-scrolling:touch}table th,table td{min-width:7rem;padding:6px}.back-top{right:12px;bottom:12px;width:42px;height:42px;font-size:24px}}</style>')
lang_summary=' · '.join(f'{LANG_LABELS.get(k,k)} {v}' for k,v in sorted(LANG_COUNTS.items()))
parts.append(f'<h1>विदेह शोध-लेख : अंक १ सँ ४४८ धरि</h1><p><b>लेखक आ सम्पादक:</b> Gajendra Thakur, Editor, Videha · ISSN 2229-547X · <a href="https://www.videha.co.in/">www.videha.co.in</a> · First Maithili Fortnightly eJournal</p><p>अन्तिम सत्यापित कॉर्पस : {len(DATA)} लेख · {lang_summary}</p>')
parts.append(f'<div class="book-tools videha-a11y-bar videha-ai-standalone" role="region" aria-label="पृष्ठ सुनबाक, अनुवाद आ सहायक तकनीक नियंत्रण" data-pagefind-ignore="all"><button type="button" id="videha-tts-toggle" aria-pressed="false">🔊 सुनू · Listen</button><button type="button" id="videha-tts-stop" hidden>⏹ रोकू</button><span id="videha-tts-status" class="videha-sr-only" role="status" aria-live="polite"></span></div><div class="book-tools" role="search" aria-label="शोध लेख खोजू"><label for="book-search"><b>शोध लेख खोजू · Search</b></label><input id="book-search" type="search" placeholder="लेखक, शीर्षक, विधा, भाषा, अंक…" autocomplete="off"><span id="book-search-status" class="book-status" role="status" aria-live="polite">{len(DATA)} लेख</span></div>')
...
for g in sorted(groups,key=str.casefold):
 ...
 for i,a in enumerate(sorted(groups[g],key=lambda x:(sortkey(x),(x.get('title') or '').casefold())),1): parts.append('<tr><td>%d</td><td>%s</td><td><a href="%s">%s</a></td><td>%s</td><td>%s · %s</td><td>%s</td><td>%s</td></tr>'%(i,esc(auth(a)),esc(a.get('url')),esc(a.get('title')),esc(a.get('language')),issue_link(a),esc(date(a)),esc(a.get('classification') or '—'),esc(pages(a))))
 parts.append('</tbody></table>')
parts.append('<div class="home-strip">⭐ <a href="https://www.videha.co.in/">VIDEHA Home · www.videha.co.in</a> <span aria-hidden="true">·</span> <a href="https://videha-ejournal.github.io/videha/">GitHub Home · videha-ejournal.github.io/videha</a></div><div class="back-top"><a href="#" aria-label="ऊपर जाउ · Back to top">↑</a></div><script src="../assets/js/videha-tts.js?v=20260818-hostfix2" defer></script><script src="../assets/js/videha-translate.js?v=20260827" defer data-videha-translate-standalone></script><script src="../assets/js/videha-access.js?v=20260827" defer></script><script>(function(){var q=document.getElementById("book-search"),st=document.getElementById("book-search-status"),rows=[].slice.call(document.querySelectorAll("table tbody tr"));function f(){var terms=(q.value||"").toLocaleLowerCase().trim().split(/\\s+/).filter(Boolean),n=0;rows.forEach(function(r){var ok=terms.every(function(t){return r.textContent.toLocaleLowerCase().indexOf(t)!==-1});r.hidden=!ok;if(ok)n++});st.textContent=(q.value?n+" मिलल · ":"")+"810 लेख"}q.addEventListener("input",f)})();</script>')
HTML.write_text(''.join(parts+['</body></html>']).replace(' <span aria-hidden="true">·</span> <a href="https://videha-ejournal.github.io/videha/">',' · ⭐ <a href="https://videha-ejournal.github.io/videha/">').replace('810 लेख',f'{len(DATA)} लेख'),encoding='utf-8')
...
doc.add_paragraph('सम्पादकीय टिप्पणी। एहि सूचीमे पूरा शोध, इतिहास, आलोचना, भाषा-विज्ञान, समाज-अध्ययन अथवा सांस्कृतिक अनुशीलनबला सामग्री राखल गेल अछि। कथा, कविता, सूचना आ मात्र अनुवादकेँ स्वतन्त्र शोध-लेख नहि गनल गेल अछि।')
