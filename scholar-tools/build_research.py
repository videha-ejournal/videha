...
def author_display(s):
    ...
    return re.sub(r'^(डॉ०|डॉ\.?|डाॅ\.?|डा\.?|प्रो\.?|आचार्य|श्री|श्रीमती|पं\.?|पण्डित)(?=\S)', r'\1 ', norm(s), flags=re.I)
...
def scan_legacy():
    rows=[]
    # Search-engine build copies and legacy gallery support pages are derivatives,
    # not independent article candidates. Excluding them keeps review conservative.
    skip={"research","templates","scholar-tools","scholar-data","server-integration","_pagefind_build","pagefind","photogallery",".git","node_modules"}
...
def write_index(articles,candidates):
    ...
    for a in sorted(articles,key=lambda x:x["publication_date"],reverse=True):
        shown_authors=[author_display(x) for x in a["authors"]]
        search_text=" ".join([a["title"],a.get("english_title") or ""," ".join(shown_authors),str(a["publication_date"]),a["issue"],a.get("language","mai"),a.get("classification","")]+list(a.get("keywords") or []))
        ...
        cards.append(f'<article class="research-card" data-year="{esc_attr(a["year"])}" data-language="{esc_attr(a.get("language","mai"))}" data-search="{esc_attr(norm(search_text).lower())}"><h2><a href="{html.escape(a["url"])}">{html.escape(a["title"])}</a></h2><p>{html.escape(", ".join(shown_authors))} · {html.escape(str(a["publication_date"]))} · {issue_link}</p><p>{html.escape(a["classification"])}</p></article>')
    ...
    language_labels={"mai":"Maithili","en":"English","sa":"Sanskrit","thethi-angika":"Thethi-Angika","bajjika":"Bajjika"}
    language_options="".join(f'<option value="{esc_attr(lang)}">{html.escape(language_labels.get(lang,lang))}</option>' for lang in languages)
    page=f'''<!doctype html><html lang="mai"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Videha Research Index</title><link rel="canonical" href="{CFG['research_base']}/"><style>body{{font-family:Georgia,'Noto Serif Devanagari',serif;max-width:960px;margin:auto;padding:2rem;line-height:1.65;color:#171717}}header{{border-bottom:3px solid #8b1a1a}}article{{padding:1rem 0;border-bottom:1px solid #ddd}}a{{color:#7d1414}}.home-strip{{display:flex;flex-wrap:wrap;gap:.8rem;padding:9px 12px;margin:10px 0;background:#faf6ee;border:1px solid #d8cfc0;font-weight:700}}.research-tools,.videha-a11y-bar{{display:flex;align-items:center;flex-wrap:wrap;gap:.6rem;margin:1rem 0;padding:.8rem;border:1px solid #d8cfc0;background:#faf6ee}}.research-tools label{{font-weight:bold}}.research-tools input{{flex:1 1 18rem;min-width:12rem}}.research-tools input,.research-tools select,.videha-tts-btn{{font:inherit;padding:.45rem .6rem}}.videha-tts-btn{{cursor:pointer}}.videha-sr-only{{position:absolute;width:1px;height:1px;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap}}.back-top{{position:fixed;right:20px;bottom:20px;width:46px;height:46px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:linear-gradient(145deg,#e53935,#8b0000);box-shadow:inset 2px 2px 5px #ff8a80,inset -3px -3px 6px #4a0000,0 3px 8px #777;color:#fff;font-size:28px;font-weight:900;z-index:10}}.back-top a{{color:#fff!important;text-decoration:none}}</style></head><body><header><h1>विदेह शोध-सूची · Videha Research Index</h1><p>{CFG['journal_title']} · ISSN {CFG['issn']}</p><p>Scholar-facing index of research, linguistics, criticism with references, history, culture, ethnography, folklore and related academic material.</p></header><div class="videha-a11y-bar videha-ai-standalone" role="region" aria-label="पृष्ठ सुनबाक, अनुवाद आ सहायक तकनीक नियंत्रण" data-pagefind-ignore="all"><button type="button" id="videha-tts-toggle" class="videha-tts-btn" aria-pressed="false" aria-label="ई पृष्ठ सुनू · Listen to this page"><span aria-hidden="true">🔊</span> <span>सुनू · Listen</span></button><button type="button" id="videha-tts-stop" class="videha-tts-btn" hidden aria-label="वाचन रोकू · Stop reading"><span aria-hidden="true">⏹</span> <span>रोकू</span></button><span id="videha-tts-status" class="videha-sr-only" role="status" aria-live="polite"></span></div><section class="research-tools" role="search" aria-label="Research articles filter"><label for="research-search">शोध लेख खोजू · Search</label><input id="research-search" type="search" placeholder="शीर्षक, लेखक, विषय, अंक वा वर्ष" autocomplete="off"><label for="research-language">भाषा</label><select id="research-language"><option value="">सभ भाषा · All</option>{language_options}</select><strong id="research-count" role="status" aria-live="polite">{len(articles)} लेख</strong></section><main id="videha-main">{body}<p id="research-empty" hidden>कोनो शोध लेख नहि भेटल · No matching research article.</p></main><footer><p>Retrospective candidates detected: {len(candidates)}. Only validated article-level records are published as Scholar pages.</p><p><a href="{CFG['site_url']}/">Videha home</a></p></footer><div class="back-top"><a href="#" aria-label="ऊपर जाउ · Back to top">↑</a></div><script>(function(){{'use strict';var q=document.getElementById('research-search'),l=document.getElementById('research-language'),cards=Array.from(document.querySelectorAll('.research-card')),count=document.getElementById('research-count'),empty=document.getElementById('research-empty');function filter(){{var terms=(q.value||'').toLocaleLowerCase().trim().split(/\\s+/).filter(Boolean),lang=l.value,shown=0;cards.forEach(function(card){{var text=card.getAttribute('data-search')||'',ok=(!lang||card.getAttribute('data-language')===lang)&&terms.every(function(term){{return text.indexOf(term)!==-1}});card.hidden=!ok;if(ok)shown++}});count.textContent=shown+' लेख · articles';empty.hidden=shown!==0}}q.addEventListener('input',filter);l.addEventListener('change',filter)}})();</script><script src="../assets/js/videha-tts.js?v=20260818-hostfix2" defer></script><script src="../assets/js/videha-translate.js?v=20260827" defer data-videha-translate-standalone></script><script src="../assets/js/videha-access.js?v=20260827" defer></script></body></html>'''
    nav='<div class="home-strip">⭐ <a href="https://www.videha.co.in/">VIDEHA Home · www.videha.co.in</a> · ⭐ <a href="https://videha-ejournal.github.io/videha/">GitHub Home · videha-ejournal.github.io/videha</a> · ⭐ <a href="https://videha-ejournal.github.io/videha/research/videha-scholar-research-book.html">शोध-लेखक समग्र अध्ययन · Alphabetical &amp; topicwise book</a></div>'
    page=page.replace("<body>", "<body>"+nav, 1).replace("</body>", nav+"</body>", 1)
